...
def fig_predictions(results, actual_arr):
    ...
    for name in ["RNN", "LSTM", "GRU", "Transformer"]:
        ax.plot(results[name]["preds"],
                label=f"{name} (MAE={results[name]['mae']:.4f})",
...
# ── figure 3: MAE bar chart (all models) ─────────────────────────────────────

def fig_mae(results):
    names  = list(results.keys())
    maes   = [results[n]["mae"] for n in names]
    colors = [BAR_COLORS[n] for n in names]
    arima_ref = results["ARIMA"]["mae"]

    fig, ax = plt.subplots(figsize=(7, 4))
    bars = ax.bar(names, maes, color=colors, alpha=0.85, edgecolor="black")
    ax.axhline(arima_ref, color="black", linestyle="--", linewidth=1, label="ARIMA baseline")
    for bar, v in zip(bars, maes):
        ax.text(bar.get_x() + bar.get_width() / 2, v + 0.001,
                f"{v:.4f}", ha="center", va="bottom", fontsize=8)
    ax.set_ylabel("MAE (scaled Close)")
    ax.set_title("Mean Absolute Error — all models")
    ax.legend(fontsize=8)
    fig.tight_layout()
    path = os.path.join(FIG_DIR, "fig_mae.pdf")
    fig.savefig(path)
    plt.close(fig)
    print(f"Saved {path}")
...
def fig_combined(results):
    ...
    data_triples = [
        (axes[0], maes,  "MAE (lower ↓)",              "MAE (scaled)"),
    ...
    axes[0].axhline(results["ARIMA"]["mae"], color="black", linestyle="--",
...
def fig_training_loss():
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Train MAE (scaled)")
    ax.set_title("Training Loss Curves (MAE per 5 epochs)")
    ax.legend(fontsize=8)
